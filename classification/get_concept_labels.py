...
# debug trước khi tokenization
print("Dataset columns:", train_dataset.column_names)
col = CFG.example_name[args.dataset]
print("example_name:", col)
example0 = train_dataset[0][col]
print("type of first item:", type(example0))
# Nếu batch map dùng batched=True, in một batch nhỏ
batch_example = train_dataset[:5][col]
print("sample batch types:", [type(x) for x in batch_example])
print("sample batch values (first 3):", batch_example[:3])
#finish fix

encoded_sim_train_dataset = train_dataset.map(
    lambda e: tokenizer_sim(
        [text if isinstance(text, str) else str(text) for text in e[CFG.example_name[args.dataset]]],
        padding=True,
        truncation=True,
        max_length=512
    ),
